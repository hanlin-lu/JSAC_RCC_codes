...
%--------------------------------------------------------------------------
    properties 
        % median fraction. Alias to medianFraction defined in AbstractBicriteriaAlg
         ...
         partitionFraction; % i.e. alpha % fraction of points to approximate from original set. Usually updated by bicriteria
    ...
    methods
        ...
        function optCluster = compute (obj, F)
            ...
            for i=1:obj.nIterations
                G=F.weightedRandomSubset(obj.beta); % only 8 points
...
%                c=c.getClusterVector(G);
                ...
                cluster = origCluster.smallest(obj.partitionFraction); % ClusterVector
